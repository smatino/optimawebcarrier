...
/**
 * Preloads the {@link SbcCpsRequest} associations required by the view layer.
 * 
 * Do not use other GraphLoaders in this GraphLoader. 
 */
@Named
@Singleton
public class SbcCpsRequestGraphLoader extends EntityGraphLoader<SbcCpsRequest, Integer> {
    // required by cglib to create a proxy around the object as we are using the @Transactional annotation
    protected SbcCpsRequestGraphLoader() {
        super();
    }

    @Inject
    public SbcCpsRequestGraphLoader(SbcCpsRequestRepository sbcCpsRequestRepository) {
        super(sbcCpsRequestRepository);
    }

    @Override
    public void loadGraph(SbcCpsRequest sbcCpsRequest) {
        loadSingular(sbcCpsRequest.getFile());
        loadSingular(sbcCpsRequest.getOlo());
    }
}